...
public class NnAcceptedStateMachineConfig extends StateMachineConfigurerAdapter<NnAcceptStatus, AcceptEvent> {
    ...
    public void configure(StateMachineTransitionConfigurer<NnAcceptStatus, AcceptEvent> transitions) throws Exception {
        transitions
                ...
                .and().withExternal()
                .event(TRADE_DONE).source(MASTER_UNKNOWN).target(SLAVE_UNKNOWN).action(machine::openSlave, machine::error)
                .and().withExternal()
                .event(TRADE_DONE).source(MASTER_OPENED).target(SLAVE_UNKNOWN).action(machine::openSlave, machine::error)
                .and().withExternal()
